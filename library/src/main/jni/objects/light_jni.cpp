...
Java_com_eje_1c_meganekko_Light_getVec3(JNIEnv * env,
    ...
    const char* char_key = env->GetStringUTFChars(key, 0);
    std::string native_key = std::string(char_key);
    OVR::Vector3f light_vec3 = light->getVec3(native_key);
    jfloatArray jvec3 = env->NewFloatArray(3);
    env->SetFloatArrayRegion(jvec3, 0, 3, reinterpret_cast<jfloat*>(&light_vec3));
    env->ReleaseStringUTFChars(key, char_key);
    return jvec3;
}
...
Java_com_eje_1c_meganekko_Light_setVec3(JNIEnv * env,
    ...
    const char* char_key = env->GetStringUTFChars(key, 0);
    std::string native_key = std::string(char_key);
    light->setVec3(native_key, OVR::Vector3f(x, y, z));
    env->ReleaseStringUTFChars(key, char_key);
}
...
Java_com_eje_1c_meganekko_Light_getVec4(JNIEnv * env,
    ...
    const char* char_key = env->GetStringUTFChars(key, 0);
    std::string native_key = std::string(char_key);
    OVR::Vector4f light_vec4 = light->getVec4(native_key);
    jfloatArray jvec4 = env->NewFloatArray(4);
    env->SetFloatArrayRegion(jvec4, 0, 4, reinterpret_cast<jfloat*>(&light_vec4));
    env->ReleaseStringUTFChars(key, char_key);
    return jvec4;
}
...
Java_com_eje_1c_meganekko_Light_setVec4(JNIEnv * env,
    ...
    const char* char_key = env->GetStringUTFChars(key, 0);
    std::string native_key = std::string(char_key);
    light->setVec4(native_key, OVR::Vector4f(x, y, z, w));
    env->ReleaseStringUTFChars(key, char_key);
}
}
