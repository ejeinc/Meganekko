...
CustomShader::CustomShader(std::string vertex_shader,
        ...
        a_position_(0), a_normal_(0), a_tex_coord_(0), u_mvp_(0), u_right_(
    ...
    program_ = BuildProgram(vertex_shader.c_str(), fragment_shader.c_str());
...
void CustomShader::addTextureKey(std::string variable_name, std::string key) {
    int location = glGetUniformLocation(program_.program, variable_name.c_str());
    texture_keys_[location] = key;
}

void CustomShader::addAttributeFloatKey(std::string variable_name,
        std::string key) {
    int location = glGetAttribLocation(program_.program, variable_name.c_str());
...
void CustomShader::render(const OVR::Matrix4f& mvp_matrix, RenderData* render_data, Material* material,
        bool right) {
