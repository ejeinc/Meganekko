 ...
 *
 *     http://www[3]pache.org/licenses/LICENSE-2.0
 *
...
static const char FRAGMENT_SHADER[] =
        "precision highp float;\n"
                ...
                "  gl_FragColor = vec4(color[0] * u_color[0] * u_opacity, color[1] * u_color[1] * u_opacity, color[2] * u_color[2] * u_opacity, color[3] * u_opacity);\n"
                "#else\n"
...
void AssimpShader::render(const OVR::Matrix4f& mv_matrix,
    ...
    if (ISSET(feature_set, AS_DIFFUSE_TEXTURE)) {
        ...
        OVR::Vector4f diffuse_color = material->getVec4("diffuse_color");
        OVR::Vector4f ambient_color = material->getVec4("ambient_color");
        glUniform4f(u_diffuse_color_, diffuse_color[0], diffuse_color[1],
                diffuse_color[2], diffuse_color[3]);
        glUniform4f(u_ambient_color_, ambient_color[0], ambient_color[1],
                ambient_color[2], ambient_color[3]);
    }
    glUniform3f(u_color_, color[0], color[1], color[2]);
