...
class CompressedTexture {
    ...
    static CompressedTexture load(InputStream stream, boolean closeStream)
        ...
        CompressedTextureLoader valid = null;
        List<CompressedTextureLoader> loaders = CompressedTextureLoader
                .getLoaders();
        synchronized (loaders) {
            for (CompressedTextureLoader loader : loaders) {
    ...
    static CompressedTextureLoader sniff(InputStream stream)
            throws IOException {
        byte[] data = readBytes(stream,
                CompressedTextureLoader.maximumHeaderLength);
