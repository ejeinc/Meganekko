...
public enum ImportSettings {
    ...
    private static EnumSet<ImportSettings> recommendedSettings = EnumSet.of(TRIANGULATE, FLIP_UV, JOIN_IDENTICAL_VERTICES,
            LIMIT_BONE_WEIGHT, SORTBY_PRIMITIVE_TYPE);
    private int mValue;

    private ImportSettings(int settings) {
        mValue = settings;
    }
     ...
     * if you want additional settings use {@link #getRecommendedSettingsWith(EnumSet<ImportSettings>) getRecommendedSettingsWith}.
     *
     * @param settings EnumSet of all import settings desired
     * @return flag in the assimp import format.
     */
    public static int getAssimpImportFlags(EnumSet<ImportSettings> settings) {
        int flags = 0;
        for (ImportSettings s : settings) {
            flags |= s.getValue();
        }
        return flags;
    }
    ...
    /**
     * Return recommended settings for simple meshes.
     *
     * @return EnumSet of recommended settings.
     */
    public static EnumSet<ImportSettings> getRecommendedSettings() {
     ...
     */
    public static EnumSet<ImportSettings> getRecommendedSettingsWith(EnumSet<ImportSettings> additionalSettings) {
     ...
     */
    public static EnumSet<ImportSettings> getRecommendedBumpmapSettings() {
        EnumSet<ImportSettings> bumpmapSettings = EnumSet.of(CALCULATE_SMOOTH_NORMALS, CALCULATE_TANGENTS);
