...
void CustomShader::render(const OVR::Matrix4f& mvp_matrix, RenderData* render_data, Material* material,
    ...
    for (auto it = uniform_vec2_keys_.begin(); it != uniform_vec2_keys_.end();
            ++it) {
        OVR::Vector2f v = render_data->material()->getVec2(it->second);
        glUniform2f(it->first, v.x, v.y);
    }

    for (auto it = uniform_vec3_keys_.begin(); it != uniform_vec3_keys_.end();
            ++it) {
        OVR::Vector3f v = render_data->material()->getVec3(it->second);
        glUniform3f(it->first, v.x, v.y, v.z);
    }

    for (auto it = uniform_vec4_keys_.begin(); it != uniform_vec4_keys_.end();
            ++it) {
        OVR::Vector4f v = render_data->material()->getVec4(it->second);
        glUniform4f(it->first, v.x, v.y, v.z, v.w);
    }

    for (auto it = uniform_mat4_keys_.begin(); it != uniform_mat4_keys_.end();
            ++it) {
        OVR::Matrix4f m = render_data->material()->getMat4(it->second);
        glUniformMatrix4fv(it->first, 1, GL_TRUE, m.M[0]);
    }
