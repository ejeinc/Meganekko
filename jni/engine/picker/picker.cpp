...
std::vector<EyePointeeHolder*> Picker::pickScene(Scene* scene, float ox,
    ...
    OVR::Matrix4f view_matrix = scene->main_camera()->transform()->getModelMatrix().InvertedHomogeneousTransform();
...
float Picker::pickSceneObject(const SceneObject* scene_object,
        const Camera* camera) {
    OVR::Matrix4f view_matrix = camera->transform()->getModelMatrix().InvertedHomogeneousTransform();
    if (scene_object->eye_pointee_holder() != 0) {
        EyePointeeHolder* eye_pointee_holder = scene_object->eye_pointee_holder();
