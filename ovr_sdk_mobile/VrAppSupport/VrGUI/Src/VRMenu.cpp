        ...
        menuHandle_t parentHandle, bool const recenter ) 
{
...

	{
		...
		for ( int i = 0; i < pairs.GetSizeI(); ++i )
		{
			...
			{
				parentHandle = HandleForName( guiSys.GetVRMenuMgr(), pair.Parms->ParentName.ToCStr() );
			}
			else if ( pair.Parms->ParentId.IsValid() )
			{
				parentHandle = HandleForId( guiSys.GetVRMenuMgr(), pair.Parms->ParentId );
			}
...
//==============================
// VRMenu::Frame
void VRMenu::Frame( OvrGuiSys & guiSys, ovrFrameInput const & vrFrame, Matrix4f const & centerViewMatrix )
{
